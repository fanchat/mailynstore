...
function verifyScrypt(password: string, storedB64: string): boolean {
  ...
  // Verify checksum: sha256 of params+salt, first 16 bytes
  ...
  // Derive 64-byte key from scrypt (scrypt-kdf derives 64 bytes)
  let hmacKey: Buffer
  try {
    hmacKey = scryptSync(password, salt, 64, { N, r, p, maxmem: 256 * 1024 * 1024 })
  } catch { return false }
  // HMAC-SHA256 of bytes 0-63 using last 32 bytes of derived key
